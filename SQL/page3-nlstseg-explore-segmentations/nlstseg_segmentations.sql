WITH
  ...
  SELECT
    *,
    ...
    COALESCE(rss_one) AS referenced_sop
  FROM
    sampled_sops)
SELECT
  segmentations.*,
  dicom_all.SeriesInstanceUID AS segmented_SeriesInstanceUID,
  dicom_all.SeriesDescription AS segmented_SeriesDescription,
  dicom_all.SeriesNumber AS segmented_SeriesNumber,
  CONCAT('https://viewer.imaging.datacommons.cancer.gov/v3/viewer/?StudyInstanceUIDs=', segmentations.StudyInstanceUID) AS viewer_url, -- so we can view all the SEG/SR in a study. 
  -- CONCAT("https://viewer.imaging.datacommons.cancer.gov/viewer/", segmentations.StudyInstanceUID,"?seriesInstanceUID=",segmentations.SeriesInstanceUID,",",dicom_all.SeriesInstanceUID) AS series_viewer_url,
    CONCAT("https://viewer.imaging.datacommons.cancer.gov/v3/viewer/?StudyInstanceUIDs=", segmentations.StudyInstanceUID,"&SeriesInstanceUIDs=",segmentations.SeriesInstanceUID,",",dicom_all.SeriesInstanceUID) AS series_viewer_url,
FROM
  coalesced_ref
JOIN
